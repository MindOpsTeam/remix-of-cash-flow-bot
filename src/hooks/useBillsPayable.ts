...
export function useBillsPayable() {
  ...
  // NÃO existe mais um "marcar como pago" aqui. Dar baixa passa pela RPC
  // `baixar_titulo` (hook useBaixaTitulo), que trava o título, aceita valor
  // parcial e lança a despesa no DRE dentro da mesma transação. O atalho antigo
  // só mudava o status: era assim que o resultado mostrava lucro que não
  // existia, e manter dois caminhos para a mesma coisa é como um deles volta a
  // ficar desatualizado.
  return {
    ...query,
    bills: query.data ?? [],
