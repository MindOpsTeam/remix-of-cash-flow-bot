...
export default function PersonalTransfers() {
  const { transfers, anticipations, transfersSummary, anticipationsSummary, isLoading } = useAsaasTransfers();
  const { subscriptions, summary: subsSummary, isLoading: subsLoading } = useAsaasSubscriptions();

  const groupedTransfers = useMemo(() => groupByDate(transfers, "scheduled_date"), [transfers]);
      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Movimentações, assinaturas e antecipações
          </p>
        </div>
