              ...
              <div className="bg-card border border-border rounded-lg divide-y divide-border">
                {invoices.map((inv) => (
                  <div key={inv.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors">
                    <div className="flex-shrink-0 h-9 w-9 rounded-full flex items-center justify-center bg-emerald-500/10 text-emerald-600">
                      <FileText className="h-4 w-4" />
                    </div>
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <p className="text-sm font-medium truncate">
                        {inv.number ? `NFS-e #${inv.number}` : "Nota Fiscal"}
                      </p>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {inv.effective_date && (
                          <span className="text-xs text-muted-foreground">
                            {new Date(inv.effective_date + "T00:00:00").toLocaleDateString("pt-BR")}
                          </span>
                        )}
                        <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${
                          inv.status === "AUTHORIZED" ? "text-revenue border-revenue/30"
                            : inv.status === "ERROR" ? "text-destructive border-destructive/30"
                            : ""
                        }`}>
                          {invoiceStatusLabels[inv.status] || inv.status}
                        </Badge>
                        {inv.error_message && (
                          <span className="text-[10px] text-destructive truncate max-w-[200px]">{inv.error_message}</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <div className="flex gap-1">
                        {inv.pdf_url && (
                          <a href={inv.pdf_url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground" title="PDF">
